...
1. const修饰指针   --- 常量指针：常指针const修饰的是指针，指针指向可以改，指针指向的值不可以更改
...
int main() {
	...
	//常量指针：常指针const修饰的是指针，指针指向可以改，指针指向的值不可以更改
	const int * p1 = &a; 
	p1 = &b; //正确
	//*p1 = 100;  报错
...
> 总结1：对于内置的数据类型（int、 double）的表达式的的运算符是不可能改变的
...
![image-20230714172315159](assets/image-20230714172315159.png)
...
![img](assets/clip_image002-1685352270568-1.png)
...
![image-20230717170733010](assets/image-20230717170733010.png)
...
* 利用虚继承可以解决菱形继承问题（继承的是两个指针的偏移量）

vbptr: virtual base pointer

vbtable: virtual base table

![image-20230717175641003](assets/image-20230717175641003.png)
...
[union](https://so.csdn.net/so/search?q=union&spm=1001.2101.3001.7020)即为联合，它是一种特殊的类。通过关键字union进行定义，一个union可以有多个数据成员。在任意时刻，联合中只能有一个数据成员可以有值。当给联合中某个成员赋值之后，该联合中的其它成员就变成未定义状态了。

# union

在C/C++程序的编写中，当多个基本数据类型或复合数据结构要占用同一片内存时，我们要使用联合体；当多种类型，多个对象，多个事物只取其一时（我们姑且通俗地称其为“n 选1”），我们也可以使用联合体来发挥其长处。
union主要是**共享内存**，分配内存以其最大的结构或对象为大小，即sizeof最大的。
